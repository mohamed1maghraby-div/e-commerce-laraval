                        ...
                        <div id="collapse_{{ $menu->route }}" 
                            ...
                            <div class="bg-white py-2 collapse-inner rounded">
                                @foreach ($menu->appearedChildren as $sub_menu)
                                    <a class="collapse-item {{ getParentShowOf($current_page) != null && (int)(getParentIdOf($current_page)+1) == $sub_menu->id ? 'active' : null }}" 
                                    href="{{ route('admin.' . $sub_menu->as) }}">{{ $menu->display_name }}</a>
                                @endforeach
                            </div>
                        </div>
                    @endif
                ...
                <li class="nav-item {{ in_array($menu->parent_show, [getParentShowOf($current_page), getParentOf($current_page)]) ? 'active' : null}}">
                    ...
                    @if ($menu->appearedChildren !== null && count($menu->appearedChildren) > 0)
                        <div id="collapse_{{ $menu->route }}" 
                            ...
                            >
                            <div class="bg-white py-2 collapse-inner rounded">
                                @foreach ($sub_menu->appearedChildren as $sub_menu)
                                @permission($menu->name)
                                    <a class="collapse-item {{ getParentShowOf($current_page) != null && (int)(getParentIdOf($param)+1) == $sub_menu->id ? 'active' : null }}" 
                                    href="{{ route('admin.' . $sub_menu->as) }}">{{ $menu->display_name }}</a>
                                @endpermission
                                @endforeach
                            </div>
