                    ...
                    @forelse ($customer_addresses as $address)
                        <tr>
                            ...
                            <td>
                                ...
                                <p class="text-gray-400">{{ $address->defaultAddress() }}<br/></p>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="btn-group btn-group-sm">
                                    ...
                                    <a href="javascript:void(0);" onclick="if(confirm('Are you sure to delete this record?')){document.getElementById('delete-address-{{ $address->id }}').submit(); }else{return false;}" class="btn btn-danger">
                                        <i class="fa fa-trash"></i>
                                    </a>
                                </div>
                                <form action="{{ route('admin.customer_addresses.destroy',$address->id) }}" method="post" id="delete-address-{{ $address->id }}" class="d-none">
                                    @csrf
                                    @method('DELETE') 
                                </form>
                            </td>
